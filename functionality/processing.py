...
def cancel_noise(input_path, noise_path):
    ...
    f_noise, _, noise_signal = manual_stft(
        noise_data_float, noise_rate, window, nperseg=M, noverlap=R
    )  # We only need the magnitude from this
    ...
    mag_denoised = np.maximum(
        beta * mag_input, mag_input - alpha * mag_noise
    )  # Use maximum to avoid negative values
